def prototypes_distribution_in_columns(ax,input_file,proto_atom,A_atom):
  ...
  # find 0.05 eV/atom index
  on_the_convex = list(filter(lambda x:x[-1]>0.06,sorted_proto_id_dleta_list))[0]
  index = sorted_proto_id_dleta_list.index(on_the_convex)
  index_005 = index-1
  ...
  #for i in graph_data_matrix:
    #print i
  #fig = plt.figure()
  barprops = dict(aspect='auto', cmap='Purples', interpolation='nearest')
  #fig, ax = plt.subplots()
  ...
  for i in range(0,30):
    if i == zero_index:
      ...
      plt.plot([0,19],[i+0.55,i+0.55],'-',color='c',linewidth = 1.5,label = '0.01 eV')
    else:
      plt.plot([0,19],[i+0.55,i+0.55],'-',color='gray',linewidth = 0.5)

  # Vertical line
  for i in range(0,19):
    plt.plot([i-0.5,i-0.5],[-0.5,30.5],'-',color='gray',linewidth = 0.5)
