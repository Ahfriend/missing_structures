...
def plot_convex(ax,input_file,proto_atom,A_atom,title=None,out_file=None,graph_num=None,proto_graph=None,ptable=None,on_convex_only=None,convex_width = 100,leg_pos =(1.05, 1.6),near_neighbor=None,ylim=None,axis_color='gray',convex_color='gray'):
  print ('graph:'+A_atom+proto_atom)
  import sys
  ...
  sys.path.append('/work/alonh/Missing_structures/data_analysis/find_neighboring_compounds')
  ...
  from column_elements import column_atoms
  ...
  indexKey = {}
  faceKey = {}
  #near neigbor color code
  if near_neighbor == True:
    atomic_number = Atomes_number_symbol_conversion(A_atom)
    col_num = col_atom_affilation(str(atomic_number))
    neighboring_columns = [col_num-1,col_num,col_num+1]
    near_elements = []
    for col_num in neighboring_columns:
      near_elements = near_elements + column_atoms(col_num)
    
   
    for element in near_elements:
      colorKey.update({element:'m'})
  ...
  for line in lines:
    ...
    if proto_graph==None and near_neighbor==None:
      if atom == proto_atom:
    ...
    if proto_graph==None and near_neighbor==True:
      if atom == proto_atom:
        colorKey.update({atom:'gainsboro'})

        # choose the color of the proto atom depended its location.
        if near_elements.count(atom)> 0:
          colorKey.update({atom:'m'})
        else:
          colorKey.update({atom:'gainsboro'})  # 
          #colorKey.update({atom:color})
...
# check if appears in aflow and create a dictioanry.
  ...
  for i in all_points:
    ...
    if icsd_id !=[]:
      ...
      try:
        mp_id = re.findall('/(([A-Z]+[a-z]*\d+)+?-mp-\d+)',path)
        print (mp_id)
        id = mp_id[0][0]
        i.append(id)
      except:
        try:
          aflow_id = re.findall('/(aflow_([A-Z]+[a-z]*)+_\d+)',path)
          print (aflow_id)
          i.append(aflow_id[0][0])
        except:
          aflow_id = re.findall('/(aflow_([A-Z]+[a-z]*)+_ICSD_\d+)',path)
          print (aflow_id)
          i.append(aflow_id[0][0])
...
#########  Plot graphs:  #########
  ...
  axins = inset_axes(ax,
                width= str(convex_width)+"%", # width = 30% of parent_bbox
 ...
 # Ticks font size
  ...
  axins.tick_params(axis='x', color=axis_color)
  axins.tick_params(axis='y', color=axis_color)
  
  # plot the 'global' convex and it plus 0.1eV
  for i in range(len(vertices)-1):
    s = np.array([vertices[i],vertices[i+1]])# The indecies of two points on the convex
    plt.plot(points[s ,0], points[s ,1], '-',color=convex_color,linewidth=0.75) # plot((x0,y0),(x1,y1))
  ...
  for j,group_name in enumerate(group_names): # runs on the diffrent gourp elements groups
    ...
    print (indexKey[group_name])
    print ('legend_name',legend_names[j])
    if graph_num != None:
      if graphNs[j]==graph_num or graphNs[j]==0:          
          plt.scatter(group[:,0],group[:,1],s=10,color=name_color_dict[group_name],fc=faceKey[group_name],marker=indexKey[group_name],label = legend_names[j])
    
    elif graph_num == None:
          plt.scatter(group[:,0],group[:,1],s=10,color=name_color_dict[group_name],fc=faceKey[group_name],marker=indexKey[group_name],label = legend_names[j])
    

  plt.legend(loc=1,bbox_to_anchor=leg_pos,scatterpoints=1,fontsize=4,frameon='False')  # first size was 1.4, 1.6
  ...
  if ylim == None:
    ymin = min(points[:,1])
    plt.ylim(ymin-0.1*abs(ymin),0.15)
